...
export function AdminDashboard({ user, data, initialTab, signOutPath }: { user: PortalUser; data: Data; initialTab: AdminTab; signOutPath: string }) {
  ...
  const [editingKir, setEditingKir] = useState<KirRecord | null>(null);
  ...
  async function saveDriver(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = event.currentTarget;
    const response = await fetch(editingDriver ? `/api/drivers/${editingDriver.id}` : "/api/drivers", { method: editingDriver ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(form))) });
    const result = await response.json() as { error?: string };
  ...
  async function saveMaintenance(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = event.currentTarget;
    const response = await fetch(editingMaintenance ? `/api/maintenance/${editingMaintenance.id}` : "/api/maintenance", { method: editingMaintenance ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(form))) });
    const result = await response.json() as { error?: string };
  ...
  async function saveFuel(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = event.currentTarget;
    const response = await fetch(editingFuel ? `/api/fuel/${editingFuel.id}` : "/api/fuel", { method: editingFuel ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(form))) });
    const result = await response.json() as { error?: string };
  ...
  async function saveKir(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = event.currentTarget;
    const response = await fetch(editingKir ? `/api/kir/${editingKir.id}` : "/api/kir", { method: editingKir ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(form))) });
    const result = await response.json() as { error?: string };
  ...
  async function saveUser(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(editingUser ? `/api/users/${editingUser.id}` : "/api/users", { method: editingUser ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(editingUser ? { ...values, action: "edit" } : values) });
    const result = await response.json() as { error?: string };
  ...
  async function manageUser(id: string, action: "reset-password" | "toggle-active") {
    if (action === "reset-password" && !window.confirm("Reset password akun ini menjadi ArmadaDIY#2026!?")) return;
  ...
  async function changeDriverStatus(driver: Driver, status: "Tersedia" | "Bertugas") {
    ...
    const response = await fetch(`/api/drivers/${driver.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "availability", status }) });
    const result = await response.json() as { error?: string };
  ...
  return (
    <main className="admin-shell">
      ...
      <section className="admin-content">
        ...
        {tab === "Pengemudi" && <DriversPanel drivers={drivers} updatingDriverId={updatingDriverId} onStatusChange={changeDriverStatus} onAdd={() => { setEditingDriver(null); setShowDriverForm(true); }} onEdit={(record) => { setEditingDriver(record); setShowDriverForm(true); }} />}
        {tab === "Pemeliharaan" && <MaintenancePanel records={data.maintenance} onAdd={() => { setEditingMaintenance(null); setShowMaintenanceForm(true); }} onEdit={(record) => { setEditingMaintenance(record); setShowMaintenanceForm(true); }} />}
        {tab === "Bahan Bakar" && <FuelPanel records={data.fuelRecords} onAdd={() => { setEditingFuel(null); setShowFuelForm(true); }} onEdit={(record) => { setEditingFuel(record); setShowFuelForm(true); }} />}
        {tab === "Jadwal KIR" && <KirPanel records={data.kirRecords} onAdd={() => { setEditingKir(null); setShowKirForm(true); }} onEdit={(record) => { setEditingKir(record); setShowKirForm(true); }} />}
        ...
        {tab === "Pengelolaan User" && user.role === "admin" && <UsersPanel users={data.users} onAdd={() => { setEditingUser(null); setShowUserForm(true); }} onEdit={(account) => { setEditingUser(account); setShowUserForm(true); }} onAction={manageUser} />}
      </section>
      {showDriverForm && <DriverModal record={editingDriver} onClose={() => { setShowDriverForm(false); setEditingDriver(null); }} onSubmit={saveDriver} />}
      {showMaintenanceForm && <MaintenanceModal record={editingMaintenance} vehicles={data.vehicles} onClose={() => { setShowMaintenanceForm(false); setEditingMaintenance(null); }} onSubmit={saveMaintenance} />}
      {showFuelForm && <FuelModal record={editingFuel} vehicles={data.vehicles} onClose={() => { setShowFuelForm(false); setEditingFuel(null); }} onSubmit={saveFuel} />}
      {showKirForm && (editingKir ? <EditKirModal record={editingKir} vehicles={data.vehicles} drivers={drivers} onClose={() => { setShowKirForm(false); setEditingKir(null); }} onSubmit={saveKir} /> : <KirModal vehicles={data.vehicles} drivers={drivers} onClose={() => setShowKirForm(false)} onSubmit={saveKir} />)}
      {showUserForm && (editingUser ? <EditUserModal account={editingUser} onClose={() => { setShowUserForm(false); setEditingUser(null); }} onSubmit={saveUser} /> : <UserModal onClose={() => setShowUserForm(false)} onSubmit={saveUser} />)}
    </main>
  );
}
...
function DriversPanel({ drivers, updatingDriverId, onStatusChange, onAdd, onEdit }: { drivers: Driver[]; updatingDriverId: string | null; onStatusChange: (driver: Driver, status: "Tersedia" | "Bertugas") => void; onAdd: () => void; onEdit: (driver: Driver) => void }) {
  return <section className="admin-section"><div className="section-tools"><div><h2>Daftar pengemudi</h2><p>Pilih status pada kartu. Halaman publik otomatis hanya menampilkan pengemudi yang tersedia.</p></div><button className="button button-dark" onClick={onAdd}>+ Tambah Pengemudi</button></div><div className="driver-grid">{drivers.map((driver) => { const availability = driver.status === "Bertugas" ? "Bertugas" : "Tersedia"; return <article key={driver.id}><span>{driver.name.charAt(0)}</span><div><h3>{driver.name}</h3><p>{driver.position ?? "Pengemudi"} · {driver.unit ?? driver.assigned_vehicle ?? "Belum ada unit"}</p><small>{driver.license_type ? `${driver.license_type}${driver.license_expiry ? ` · berlaku s.d. ${formatDate(driver.license_expiry)}` : ""}` : "SIM belum dicatat"}</small></div><label className="driver-status-control"><span className="sr-only">Status {driver.name}</span><select aria-label={`Status ${driver.name}`} className={`status-select status-${slug(availability)}`} value={availability} disabled={updatingDriverId === driver.id} onChange={(event) => onStatusChange(driver, event.target.value as "Tersedia" | "Bertugas")}><option value="Tersedia">Tersedia</option><option value="Bertugas">Sedang Bertugas</option></select>{updatingDriverId === driver.id && <small>Menyimpan…</small>}</label><button className="edit-button" type="button" onClick={() => onEdit(driver)}>Edit</button></article>; })}</div>{drivers.length === 0 && <p className="data-note">Belum ada data pengemudi. Gunakan tombol Tambah Pengemudi untuk membuat data pertama.</p>}</section>;
}

function DriverModal({ record, onClose, onSubmit }: { record: Driver | null; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="driver-modal" role="dialog" aria-modal="true" aria-labelledby="driver-modal-title"><header><h2 id="driver-modal-title">{record ? "Edit Pengemudi" : "Tambah Pengemudi"}</h2><button type="button" onClick={onClose} aria-label="Tutup formulir">×</button></header><form onSubmit={onSubmit}><div className="driver-form-grid"><label><span>Nama *</span><input name="name" required autoFocus defaultValue={record?.name ?? ""} placeholder="Budi Santoso" /></label><label><span>NIP</span><input name="nip" inputMode="numeric" defaultValue={record?.nip ?? ""} placeholder="19850101 201001 1 001" /></label><label><span>Jabatan</span><input name="position" defaultValue={record?.position ?? ""} placeholder="Analis" /></label><label><span>Unit Kerja</span><input name="unit" defaultValue={record?.unit ?? ""} placeholder="Bagian Umum" /></label><label><span>No. SIM</span><input name="licenseNumber" defaultValue={record?.license_number ?? ""} /></label><label><span>Jenis SIM</span><select name="licenseType" defaultValue={record?.license_type ?? "SIM B1"}><option>SIM A</option><option>SIM B1</option><option>SIM B2</option><option>SIM C</option></select></label><label><span>Berlaku SIM</span><input name="licenseExpiry" type="date" defaultValue={record?.license_expiry ?? ""} /></label><label><span>No. HP</span><input name="phone" inputMode="tel" defaultValue={record?.phone ?? ""} placeholder="0812..." /></label><label><span>Status Ketersediaan</span><select name="status" defaultValue={record?.status ?? "Tersedia"}><option value="Tersedia">Tersedia</option><option value="Bertugas">Sedang Bertugas</option></select></label><label><span>URL Foto</span><input name="photoUrl" type="url" defaultValue={record?.photo_url ?? ""} placeholder="https://..." /></label><label className="full-field"><span>Alamat</span><textarea name="address" rows={3} defaultValue={record?.address ?? ""} placeholder="Jl. ..." /></label></div><footer><button className="button modal-cancel" type="button" onClick={onClose}>Batal</button><button className="button modal-save" type="submit">{record ? "Simpan Perubahan" : "Simpan"}</button></footer></form></section></div>;
}

function MaintenancePanel({ records, onAdd, onEdit }: { records: MaintenanceRecord[]; onAdd: () => void; onEdit: (record: MaintenanceRecord) => void }) {
  return <section className="admin-section"><div className="section-tools"><div><h2>Riwayat pemeliharaan</h2><p>Servis, perbaikan, biaya, dan odometer armada.</p></div><button className="button button-dark" onClick={onAdd}>+ Tambah Pemeliharaan</button></div>{records.length ? <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Kendaraan</th><th>Tanggal</th><th>Jenis</th><th>Bengkel</th><th>Kilometer</th><th>Biaya</th><th>Status</th><th>Aksi</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td><strong>{record.vehicle_name}</strong><small>{record.plate_number}</small></td><td>{formatDate(record.service_date)}</td><td>{record.service_type}</td><td>{record.vendor ?? "Belum ditentukan"}</td><td>{record.odometer.toLocaleString("id-ID")} km</td><td>{formatRupiah(record.cost)}</td><td><StatusPill value={record.status} /></td><td><button className="edit-button" type="button" onClick={() => onEdit(record)}>Edit</button></td></tr>)}</tbody></table></div> : <div className="empty-admin-state"><strong>Belum ada riwayat pemeliharaan</strong><p>Tambahkan servis atau perbaikan kendaraan melalui tombol di atas.</p></div>}</section>;
}

function MaintenanceModal({ record, vehicles, onClose, onSubmit }: { record: MaintenanceRecord | null; vehicles: Vehicle[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  const today = new Date().toISOString().slice(0, 10);
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="driver-modal maintenance-modal" role="dialog" aria-modal="true" aria-labelledby="maintenance-modal-title"><header><h2 id="maintenance-modal-title">{record ? "Edit Pemeliharaan" : "Tambah Pemeliharaan"}</h2><button type="button" onClick={onClose} aria-label="Tutup formulir">×</button></header><form onSubmit={onSubmit}><div className="driver-form-grid"><label><span>Kendaraan *</span><select name="vehicleId" required defaultValue={record?.vehicle_id ?? ""}><option value="" disabled>-- Pilih Kendaraan --</option>{vehicles.map((vehicle) => <option value={vehicle.id} key={vehicle.id}>{vehicle.plate_number} · {vehicle.brand} {vehicle.model}</option>)}</select></label><label><span>Tanggal</span><input name="serviceDate" type="date" required defaultValue={record?.service_date ?? today} /></label><label><span>Jenis</span><select name="serviceType" defaultValue={record?.service_type ?? "Servis Rutin"}><option>Servis Rutin</option><option>Perbaikan</option><option>Ganti Oli</option><option>Ban</option><option>Kelistrikan</option><option>Body Repair</option><option>Lainnya</option></select></label><label><span>Status</span><select name="status" defaultValue={record?.status ?? "Diajukan"}><option>Diajukan</option><option>Dijadwalkan</option><option>Dikerjakan</option><option>Selesai</option><option>Dibatalkan</option></select></label><label><span>Bengkel</span><input name="vendor" defaultValue={record?.vendor ?? ""} placeholder="Nama bengkel" /></label><label><span>Kilometer</span><input name="odometer" type="number" min="0" step="1" defaultValue={record?.odometer ?? 0} /></label><label><span>Biaya (Rp)</span><input name="cost" type="number" min="0" step="1" defaultValue={record?.cost ?? 0} /></label><label className="full-field"><span>Deskripsi</span><textarea name="notes" rows={3} defaultValue={record?.notes ?? ""} placeholder="Detail perbaikan..." /></label></div><footer><button className="button modal-cancel" type="button" onClick={onClose}>Batal</button><button className="button modal-save" type="submit">{record ? "Simpan Perubahan" : "Simpan"}</button></footer></form></section></div>;
}

function FuelPanel({ records, onAdd, onEdit }: { records: FuelRecord[]; onAdd: () => void; onEdit: (record: FuelRecord) => void }) {
  return <section className="admin-section"><div className="section-tools"><div><h2>Riwayat pengisian BBM</h2><p>Penggunaan bahan bakar, kilometer, dan biaya per kendaraan.</p></div><button className="button button-dark" onClick={onAdd}>+ Tambah Pengisian BBM</button></div>{records.length ? <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Kendaraan</th><th>Tanggal</th><th>Jenis BBM</th><th>SPBU</th><th>Liter</th><th>Harga/Liter</th><th>Kilometer</th><th>Total</th><th>Bukti</th><th>Aksi</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td><strong>{record.vehicle_name}</strong><small>{record.plate_number}</small></td><td>{formatDate(record.fill_date)}</td><td>{record.fuel_type}</td><td>{record.station ?? "Belum diisi"}</td><td>{record.liters.toLocaleString("id-ID", { maximumFractionDigits: 2 })} L</td><td>{formatRupiah(record.price_per_liter)}</td><td>{record.odometer.toLocaleString("id-ID")} km</td><td><strong>{formatRupiah(record.total_cost)}</strong></td><td>{record.receipt_url ? <a className="receipt-link" href={record.receipt_url} target="_blank" rel="noreferrer">Lihat struk ↗</a> : "—"}</td><td><button className="edit-button" type="button" onClick={() => onEdit(record)}>Edit</button></td></tr>)}</tbody></table></div> : <div className="empty-admin-state"><strong>Belum ada riwayat pengisian BBM</strong><p>Tambahkan transaksi bahan bakar melalui tombol di atas.</p></div>}</section>;
}

function FuelModal({ record, vehicles, onClose, onSubmit }: { record: FuelRecord | null; vehicles: Vehicle[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  const today = new Date().toISOString().slice(0, 10);
  const [liters, setLiters] = useState(String(record?.liters ?? 0));
  const [pricePerLiter, setPricePerLiter] = useState(String(record?.price_per_liter ?? 0));
  const total = Math.round((Number(liters) || 0) * (Number(pricePerLiter) || 0));
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="driver-modal fuel-modal" role="dialog" aria-modal="true" aria-labelledby="fuel-modal-title"><header><h2 id="fuel-modal-title">{record ? "Edit Pengisian BBM" : "Tambah Pengisian BBM"}</h2><button type="button" onClick={onClose} aria-label="Tutup formulir">×</button></header><form onSubmit={onSubmit}><div className="driver-form-grid"><label><span>Kendaraan *</span><select name="vehicleId" required defaultValue={record?.vehicle_id ?? ""}><option value="" disabled>-- Pilih Kendaraan --</option>{vehicles.map((vehicle) => <option value={vehicle.id} key={vehicle.id}>{vehicle.plate_number} · {vehicle.brand} {vehicle.model}</option>)}</select></label><label><span>Tanggal</span><input name="fillDate" type="date" required defaultValue={record?.fill_date ?? today} /></label><label><span>Jenis BBM</span><select name="fuelType" defaultValue={record?.fuel_type ?? "Pertalite"}><option>Pertalite</option><option>Pertamax</option><option>Pertamax Turbo</option><option>Dexlite</option><option>Pertamina Dex</option><option>Solar</option><option>Listrik</option><option>Lainnya</option></select></label><label><span>Lokasi SPBU</span><input name="station" defaultValue={record?.station ?? ""} placeholder="Pertamina Jl. Sudirman" /></label><label><span>Liter</span><input name="liters" type="number" min="0" step="0.01" value={liters} onChange={(event) => setLiters(event.target.value)} /></label><label><span>Harga / Liter (Rp)</span><input name="pricePerLiter" type="number" min="0" step="1" value={pricePerLiter} onChange={(event) => setPricePerLiter(event.target.value)} /></label><label><span>Kilometer</span><input name="odometer" type="number" min="0" step="1" defaultValue={record?.odometer ?? 0} /></label><label><span>URL Bukti Struk</span><input name="receiptUrl" type="url" defaultValue={record?.receipt_url ?? ""} placeholder="https://..." /></label><div className="fuel-total full-field"><span>Total Biaya:</span><strong>{formatRupiah(total)}</strong></div></div><footer><button className="button modal-cancel" type="button" onClick={onClose}>Batal</button><button className="button modal-save" type="submit">{record ? "Simpan Perubahan" : "Simpan"}</button></footer></form></section></div>;
}

function KirPanel({ records, onAdd, onEdit }: { records: KirRecord[]; onAdd: () => void; onEdit: (record: KirRecord) => void }) {
  const totals = {
  ...
  return <section className="admin-section"><div className="section-tools"><div><h2>Pemantauan Uji KIR</h2><p>Masa berlaku, jadwal pengujian berikutnya, dan status kelayakan kendaraan.</p></div><button className="button button-dark" onClick={onAdd}>+ Tambah Data KIR</button></div><div className="kir-summary"><article><span>Aman</span><strong>{totals.aman}</strong><small>Lebih dari 30 hari</small></article><article className="warning"><span>Warning</span><strong>{totals.warning}</strong><small>Jatuh tempo dalam 30 hari</small></article><article className="expired"><span>Kedaluwarsa</span><strong>{totals.expired}</strong><small>Perlu segera diuji</small></article></div>{records.length ? <div className="data-table-wrap"><table className="data-table kir-table"><thead><tr><th>Kendaraan</th><th>Dokumen KIR</th><th>Hasil Uji</th><th>Pengemudi</th><th>Operasional</th><th>Uji Terakhir</th><th>Jadwal Berikutnya</th><th>Sisa Berlaku</th><th>Aksi</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td><strong>{record.brand_model}</strong><small>{record.plate_number} · {record.vehicle_type} {record.vehicle_year ?? ""}</small></td><td><code>{record.test_number}</code><small>Rangka: {record.chassis_number ?? "—"}<br />Mesin: {record.engine_number ?? "—"}</small></td><td><StatusPill value={record.test_result} /></td><td>{record.driver_name ?? "Belum ditetapkan"}</td><td><strong>{record.vehicle_status}</strong><small>{record.vehicle_location ?? "Lokasi belum diisi"}</small></td><td>{formatDate(record.last_test_date)}</td><td><strong>{formatDate(record.next_test_date)}</strong><small>Berlaku s.d. {formatDate(record.valid_until)}</small></td><td><StatusPill value={record.status_category} /><small className="remaining-days">{record.remaining_days < 0 ? `${Math.abs(record.remaining_days)} hari terlewat` : `${record.remaining_days} hari lagi`}</small></td><td><button className="edit-button" type="button" onClick={() => onEdit(record)}>Edit</button></td></tr>)}</tbody></table></div> : <div className="empty-admin-state"><strong>Belum ada data Uji KIR</strong><p>Tambahkan dokumen dan jadwal KIR kendaraan melalui tombol di atas.</p></div>}</section>;
}
...
function EditKirModal({ record, vehicles, drivers, onClose, onSubmit }: { record: KirRecord; vehicles: Vehicle[]; drivers: Driver[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  const [validUntil, setValidUntil] = useState(record.valid_until);
  const remainingDays = daysUntil(validUntil);
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="driver-modal kir-modal" role="dialog" aria-modal="true" aria-labelledby="edit-kir-title">
      <header><h2 id="edit-kir-title">Edit Data Uji KIR</h2><button type="button" onClick={onClose} aria-label="Tutup formulir">×</button></header>
      <form onSubmit={onSubmit}><div className="driver-form-grid">
        <label className="full-field"><span>Pilih Kendaraan *</span><select name="vehicleId" required defaultValue={record.vehicle_id}>{vehicles.map((vehicle) => <option value={vehicle.id} key={vehicle.id}>{vehicle.plate_number} · {vehicle.brand} {vehicle.model}</option>)}</select></label>
        <label><span>Nomor Polisi *</span><input name="plateNumber" required defaultValue={record.plate_number} /></label>
        <label><span>Nomor Uji KIR *</span><input name="testNumber" required defaultValue={record.test_number} /></label>
        <label><span>Nomor Rangka</span><input name="chassisNumber" defaultValue={record.chassis_number ?? ""} /></label>
        <label><span>Nomor Mesin</span><input name="engineNumber" defaultValue={record.engine_number ?? ""} /></label>
        <label><span>Jenis Kendaraan *</span><input name="vehicleType" required defaultValue={record.vehicle_type} /></label>
        <label><span>Merek / Model *</span><input name="brandModel" required defaultValue={record.brand_model} /></label>
        <label><span>Tahun Kendaraan</span><input name="vehicleYear" type="number" min="1900" max="2100" defaultValue={record.vehicle_year ?? ""} /></label>
        <label><span>Status Hasil Uji *</span><select name="testResult" defaultValue={record.test_result}><option>Lulus</option><option>Tidak Lulus</option></select></label>
        <label><span>Tanggal Uji Terakhir *</span><input name="lastTestDate" type="date" required defaultValue={record.last_test_date} /></label>
        <label><span>Masa Berlaku KIR *</span><input name="validUntil" type="date" required value={validUntil} onChange={(event) => setValidUntil(event.target.value)} /></label>
        <label><span>Nama Pengemudi</span><select name="driverName" defaultValue={record.driver_name ?? ""}><option value="">-- Belum ditetapkan --</option>{drivers.map((driver) => <option value={driver.name} key={driver.id}>{driver.name}</option>)}</select></label>
        <label><span>Status Kendaraan</span><select name="vehicleStatus" defaultValue={record.vehicle_status}><option>Operasional</option><option>Cadangan</option><option>Rusak</option></select></label>
        <label className="full-field"><span>Lokasi Kendaraan</span><input name="vehicleLocation" defaultValue={record.vehicle_location ?? ""} /></label>
        <div className="kir-auto full-field"><div><span>Jadwal Uji Berikutnya</span><strong>{formatDate(validUntil)}</strong></div><div><span>Sisa Hari Berlaku</span><strong>{remainingDays < 0 ? `${Math.abs(remainingDays)} hari terlewat` : `${remainingDays} hari lagi`}</strong></div><StatusPill value={kirCategory(remainingDays)} /></div>
      </div><footer><button className="button modal-cancel" type="button" onClick={onClose}>Batal</button><button className="button modal-save" type="submit">Simpan Perubahan</button></footer></form>
    </section>
  </div>;
}

function UsersPanel({ users, onAdd, onEdit, onAction }: { users: ManagedUser[]; onAdd: () => void; onEdit: (account: ManagedUser) => void; onAction: (id: string, action: "reset-password" | "toggle-active") => void }) {
  return <section className="admin-section"><div className="section-tools"><div><h2>Pengelolaan pengguna</h2><p>Atur akun, peran, status akses, dan reset password pengemudi.</p></div><button className="button button-dark" onClick={onAdd}>+ Tambah User</button></div><div className="account-security-note"><strong>Password awal driver: ArmadaDIY#2026!</strong><span>Segera ganti setelah akun diserahkan. Admin tetap masuk menggunakan email Clerk.</span></div><div className="data-table-wrap"><table className="data-table user-table"><thead><tr><th>Pengguna</th><th>Username</th><th>Peran</th><th>Status</th><th>Login Terakhir</th><th>Aksi</th></tr></thead><tbody>{users.map((account) => <tr key={account.id}><td><strong>{account.displayName}</strong><small>{account.role === "admin" ? account.email : "Akun operasional pengemudi"}</small></td><td><code>{account.username}</code></td><td><span className={`role-badge role-${account.role}`}>{account.role === "admin" ? "Administrator" : "Driver"}</span></td><td><StatusPill value={account.active ? "Aktif" : "Tidak Aktif"} /></td><td>{account.lastSignInAt ? new Date(account.lastSignInAt).toLocaleString("id-ID", { dateStyle: "medium", timeStyle: "short" }) : "Belum pernah"}</td><td>{account.role === "driver" ? <div className="user-actions"><button onClick={() => onEdit(account)}>Edit</button><button onClick={() => onAction(account.id, "reset-password")}>Reset Password</button><button className={account.active ? "deactivate" : "activate"} onClick={() => onAction(account.id, "toggle-active")}>{account.active ? "Nonaktifkan" : "Aktifkan"}</button></div> : <span className="admin-managed-note">Dikelola lewat Clerk</span>}</td></tr>)}</tbody></table></div></section>;
}

function UserModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="driver-modal user-modal" role="dialog" aria-modal="true" aria-labelledby="user-modal-title"><header><h2 id="user-modal-title">Tambah User Driver</h2><button type="button" onClick={onClose} aria-label="Tutup formulir">×</button></header><form onSubmit={onSubmit}><div className="driver-form-grid"><label><span>Nama Lengkap *</span><input name="displayName" required autoFocus placeholder="Nama pengemudi" /></label><label><span>Username *</span><input name="username" required minLength={3} maxLength={32} pattern="[a-z0-9._-]+" autoCapitalize="none" placeholder="nama singkat" /></label><label className="full-field"><span>Password Awal</span><input name="password" type="text" minLength={10} defaultValue="ArmadaDIY#2026!" /></label><div className="account-security-note full-field"><strong>Akses role Driver</strong><span>Hanya menu Pemeliharaan, Bahan Bakar, dan Jadwal KIR.</span></div></div><footer><button className="button modal-cancel" type="button" onClick={onClose}>Batal</button><button className="button modal-save" type="submit">Buat Akun</button></footer></form></section></div>;
}

function EditUserModal({ account, onClose, onSubmit }: { account: ManagedUser; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="driver-modal user-modal" role="dialog" aria-modal="true" aria-labelledby="edit-user-title">
      <header><h2 id="edit-user-title">Edit User Driver</h2><button type="button" onClick={onClose} aria-label="Tutup formulir">×</button></header>
      <form onSubmit={onSubmit}><div className="driver-form-grid">
        <label><span>Nama Lengkap *</span><input name="displayName" required autoFocus defaultValue={account.displayName} /></label>
        <label><span>Username *</span><input name="username" required minLength={3} maxLength={32} pattern="[a-z0-9._-]+" autoCapitalize="none" defaultValue={account.username} /></label>
        <div className="account-security-note full-field"><strong>Password tidak diubah</strong><span>Gunakan tombol Reset Password di tabel jika password perlu dikembalikan ke password awal yang aman.</span></div>
      </div><footer><button className="button modal-cancel" type="button" onClick={onClose}>Batal</button><button className="button modal-save" type="submit">Simpan Perubahan</button></footer></form>
    </section>
  </div>;
}
